...
public class SikuiliUtils {
	...
	static Region getDeviceRegion() {
		...
		///////////////////////////
		try {
			devicePoint = Target.obj.getDeviceRegion();
			System.out.println("~~~~~~~~Point from RMI: "+devicePoint.getX()+" "+devicePoint.getY());
//			Region region = SikuiliUtils.getDeviceRegion();
//			System.out.println("~~~~~~~~Point from Sikulli: "+region.x+" "+region.y);
//			obj.tapOnMiddle(region.x, region.y);
...
//		}
		...
		if(ResourceUtils.getProfileName().equals("Mobile")) {
			region = new Region(devicePoint.x - 390, devicePoint.y - 810, 480, 800);
		} else if(ResourceUtils.getProfileName().equals("Wearable")) {
			if(ResourceUtils.getScreenWidth().equals(ResourceUtils.getScreenHeight())) {
				region = new Region(devicePoint.x - 161, devicePoint.y - 370, 360, 360);
			} else {
				region = new Region(devicePoint.x - 161, devicePoint.y - 490, 360, 480);
			}
		}
...
//		region.mouseUp();
		try {
			Target.obj.tapOnMiddle(((region.x+region.w)/2), ((region.y+region.h)/2));
		} catch (RemoteException e) {
