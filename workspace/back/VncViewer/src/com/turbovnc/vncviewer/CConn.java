  ...
  public CConn(VncViewer viewer_, Socket sock_) {
	...
	t.schedule(new TimerTask() {
		public void run() {
			if (viewport != null && 
				pendingUpdate == true && 
				(getTime() - titleTimer) > 1.5) {
				if(!viewport.getCurrentTitle().contains("[K]")) {
				   viewport.updateTitle(true);
				}
			}
...
//    }
  ...
  public void framebufferUpdateStart() {
    ...
    if((getTime() - titleTimer) < 3.0) {
	    if (viewport != null &&	
	        viewport.getCurrentTitle().contains("[K]")) {
	    	viewport.updateTitle(false);
	    }
    }
    
    //System.out.println("framebufferUpdateStart() -> " + viewport.getCurrentTitle()); 
  }
  ...
  public Image getCurrentClientScreenImage() {
	  return desktop.getCurrentPlatformPixelBuffer().getImage();
  }
  ...
  private void checkEncodings() {
    if (encodingChange && (writer() != null)) {
      ...
      if (viewport != null) {
        viewport.updateTitle();
        System.out.println("checkEncodings() -> ");
      }
    }
  }
  ...
  double titleTimer = Double.MAX_VALUE;
}
