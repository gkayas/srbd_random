...
public class VncViewer extends javax.swing.JApplet
  ...
  public static final String PRODUCT_NAME = "VNC Viewer";
        ...
        if (++i < argv.length) {
          if (!argv[i].equalsIgnoreCase(System.getProperty("os.arch"))) {
            reportException(new WarningException("You must use a " +
              argv[i] + " Java Runtime Environment with this version of VNC."));
            exit(1);
          }
        }
        continue;
  ...
  public static void usage() {
    String usage = ("\n" +
                    ...
                    "Password.)\n\n" +
                    "The parameters are:\n\n");
    System.err.println("\nVNC Viewer v" + version + " (build " + build +
  ...
  void reportException(Exception e, boolean reconnect) {
    ...
    if (e instanceof WarningException) {
      ...
      title = "VNC Viewer : Error";
      System.out.println(msg);
    } else {
      if (!(e instanceof SystemException))
        msg = e.toString();
      title = "VNC Viewer : Unexpected Error";
      e.printStackTrace();
    }
    if (embed.getValue()) {
      getContentPane().removeAll();
  ...
  static void showAbout(Component comp) {
    ...
    JDialog dlg = pane.createDialog(comp, "About VNC Viewer");
  ...
  "Always show the \"VNC Connection\" dialog even if the server has " +
  ...
  "if SSH tunneling is enabled (the \"VNC Connection\" dialog is " +
  ...
  "VNC Server, use the vncconnect program.", false);
  ...
  "session.  If this option is disabled and the VNC Server is using " +
  ...
  "If the VNC Viewer is being run as an applet, display its output to " +
  ...
  "Synchronize the local clipboard with the clipboard of the VNC session " +
  "whenever the latter changes.", true);

  static BoolParameter sendClipboard
  = new BoolParameter("SendClipboard",
  "Synchronize the VNC session clipboard with the local clipboard " +
  "whenever the latter changes.", true);
  ...
  "connecting to a VNC server, but this option can be useful when " +
  ...
  "later of the VNC Server also supports Compression Level 2 when JPEG is " +
  ...
  "In v1.2 or later of the VNC Server, compression levels 5-7 map to " +
  ...
  "causes the VNC Server to use more CPU time and much more memory.",
  1, 0, 9);

  public static BoolParameter compatibleGUI
  = new BoolParameter("CompatibleGUI",
  "Normally, the VNC Viewer GUI exposes only the settings that are " +
  "useful for VNC servers.  Enabling this option will change the " +
  ...
  "Normally, VNC and compatible servers will send only changes to the " +
  ...
  "VNC connection info (.vnc) files.  For instance, you can generate " +
  "an ASCII hex VNC password on the VNC server machine by executing\n " +
  "'cat {VNC_password_file} | xxd -c 256 -ps' or\n " +
  "'echo {unencrypted_password} | /opt/VNC/bin/vncpasswd -f | xxd -c 256 -ps'\n " +
  ...
  "client is automatically used.  See the VNC User's Guide for more " +
  "details.", false);
  ...
  "VNC includes an internal profiling system that can be used to display " +
