 ...
 */
public class CustomOverlayPrompt extends OverlayTransparentWindow implements EventSubject {

  final static float MIN_DARKER_FACTOR = 0.6f;
  final static long MSG_DISPLAY_TIME = 2000;
  final static long WIN_FADE_IN_TIME = 200;
  static final Font fontMsg = new Font("Arial", Font.PLAIN, 20);
  ...
  public CustomOverlayPrompt(IScreen scr, EventObserver ob) {
    super();
    init(scr, ob);
  }
  ...
  private void captureScreen(IScreen scr) {
    ScreenImage simg = scr.capture(this.getX(),this.getY(),this.getWidth(),this.getHeight());
//	  ScreenImage simg = scr.capture();
  ...
  void drawMessage(Graphics2D g2d) {
    ...
    Rectangle ubound = new Rectangle(this.getX(), this.getY(),this.getWidth(),this.getHeight());
...
//    }
    //Rectangle bound = Screen.getBounds(i);
    int cx = ubound.x + (ubound.width - sw) / 2 - ubound.x;
    int cy = ubound.y + (ubound.height - sh) / 2 - ubound.y;
    g2d.drawString(promptMsg, cx, cy);
  }
