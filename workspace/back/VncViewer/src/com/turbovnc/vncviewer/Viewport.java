...
public class Viewport extends JFrame {
	...
	private CustomFrame remote;
//	private UIAutomatorControlsFrame automatorControls;
	public Viewport(CConn cc_) {
		...
		codeGenerator = new TizenCodeGenerator(cc_);
		getContentPane().setLayout(new FlowLayout());
		tb = new Toolbar(cc, codeGenerator, this);
		add(tb, BorderLayout.PAGE_START);

		getContentPane().add(sp);

		bottom_tb = new BottomToolbar(cc, codeGenerator);
		add(bottom_tb, BorderLayout.PAGE_END);
		...
		// NOTE: If Lion FS mode is enabled, then the viewport is only created
		// once
		// as a non-full-screen viewport, so we tell showToolbar() to ignore the
		// full-screen state.
		showToolbar(cc.showToolbar, canDoLionFS);

		sp.setSize(cc.desktop.scaledWidth, cc.desktop.scaledHeight);
		...
		if(ReadConfigUtil.getProfileName().toLowerCase().equals("tv")){
			remote = new CustomFrame(this, codeGenerator);
		}

//		automatorControls = new UIAutomatorControlsFrame(this);
		...
		addComponentListener(new ComponentAdapter() {
			public void componentResized(ComponentEvent e) {
				if (cc.opts.scalingFactor == Options.SCALE_AUTO || cc.opts.scalingFactor == Options.SCALE_FIXEDRATIO) {
				...
				} else if (cc.opts.desktopSize.mode == Options.SIZE_AUTO && !cc.firstUpdate
				...
				if (cc.desktop.cursor != null) {
					...
					if (cursor.hotspot != null) {
						// hotspot will be null until the first cursor update is
						// received
						// from the server.
						cc.setCursor(cursor.width(), cursor.height(), cursor.hotspot, (int[]) cursor.data, cursor.mask);
					}
				}
				...
				boolean isRotate = isUIRotate(ReadConfigUtil.getRotation());
				if(ReadConfigUtil.getProfileName().equals("Mobile")) {
					if(!isRotate){
						e.getComponent().setSize(490, 933);
//						paddingX = 
//						paddingY = 
					}else{
						e.getComponent().setSize(810, 613);
//						paddingX = 
//						paddingY = 
					}
					...
					if(ReadConfigUtil.getScreenWidth().equals(ReadConfigUtil.getScreenHeight())) {
						e.getComponent().setSize(490, 490);
					} else {
						if(!isRotate){
							e.getComponent().setSize(490, 610);
//							paddingX = 
//							paddingY = 
						}else{
							e.getComponent().setSize(490, 490);
...
//							paddingY = 
				...
				} else if(ReadConfigUtil.getProfileName().equals("TV")) {
		 			e.getComponent().setSize(970, 673);
//					paddingX = 
//					paddingY = 
				}
			...
			private boolean isUIRotate(String r){
				int rotation = Integer.parseInt(r);
				return !((rotation>=0 && rotation<45) || (rotation<=0 && rotation>-45) || (rotation>135 && rotation<=180) || (rotation<-135 && rotation>=-180));
			}

			public void componentMoved(ComponentEvent e) {
				if(cc.captureMode){
					setBounds(lastX, lastY, getWidth(), getHeight());
				 }else{
					 lastX = cc.viewport.getX();
					 lastY = cc.viewport.getY();
					 if(ReadConfigUtil.getProfileName().toLowerCase().equals("tv")){
						 remote.setLocation(e.getComponent().getX()-250, e.getComponent().getY()+50);
//						 automatorControls.setLocation((e.getComponent().getX() + getWidth()), e.getComponent().getY()+50);
					 }
...
//		}
	...
	public Dimension getBorderSize() {
		...
		Insets vpInsets = VncViewer.insets;
		Dimension borderSize = new Dimension(vpInsets.left + vpInsets.right, vpInsets.top + vpInsets.bottom);
		if (cc.showToolbar)
	...
	void enableLionFS() {
		try {
			...
			Object proxy = Proxy.newProxyInstance(fsListenerClass.getClassLoader(), new Class[] { fsListenerClass },
					fsHandler);
	...
	public void updateTitle() {
		...
		currentTitle = "Device Interface [ " + ReadConfigUtil.getProfileName() + "-" +ReadConfigUtil.getPlatformVersion()+ "-" +ReadConfigUtil.getProfileType() +" ]";
		setTitle(currentTitle);
		setLocation(0, 0);
	}

	public void updateTitle(boolean isK) {

		if(isK) {
			currentTitle =  "Device Interface [ " + ReadConfigUtil.getProfileName() + "-" +ReadConfigUtil.getPlatformVersion()+ "-" +ReadConfigUtil.getProfileType() +" ] " + "[K]";
		} else {
			currentTitle = "Device Interface [ " + ReadConfigUtil.getProfileName() + "-" +ReadConfigUtil.getPlatformVersion()+ "-" +ReadConfigUtil.getProfileType() +" ]";
		}
	...
	public static boolean isHelperAvailable() {
		if (!triedHelperInit) {
			try {
				...
				vlog.info("WARNING: Could not find VNC Helper JNI library.  If it is in a");
				...
				vlog.info("WARNING: Could not initialize VNC Helper JNI library:");
	...
	public void x11FullScreenHelper(boolean on) {
		if (isHelperAvailable()) {
			try {
				...
				vlog.info("WARNING: Could not invoke x11FullScreen() from VNC Helper.");
				vlog.info("  Full-screen mode may not work correctly.");
	...
	public void grabKeyboardHelper(boolean on, boolean force) {
		if (isHelperAvailable()) {
			try {
				...
				vlog.info("WARNING: Could not invoke grabKeyboard() from VNC Helper.");
	...
	public void setupExtInputHelper() {
		if (isHelperAvailable() && cc.cp.supportsGII && x11dpy == 0) {
			try {
				...
				vlog.info("WARNING: Could not invoke setupExtInput() from VNC Helper.");
	...
	public void cleanupExtInputHelper() {
		if (isHelperAvailable() && x11dpy != 0) {
			try {
				...
				vlog.info("WARNING: Could not invoke cleanupExtInput() from VNC Helper.");
				vlog.info("  Extended input device support will be disabled.");
				helperAvailable = false;
			} catch (java.lang.Exception e) {
				vlog.info("WARNING: Could not invoke cleanupExtInput() from VNC Helper:");
	...
	void assignInputDevice(int deviceOrigin) {
		if (devices == null) {
			vlog.eidebug("Attempted to assign GII device ID " + deviceOrigin + " to non-existent device");
			return;
		}
		for (Iterator<ExtInputDevice> i = devices.iterator(); i.hasNext();) {
			...
			if (dev.remoteID == 0) {
				...
				vlog.info("Successfully created device " + deviceOrigin + " (" + dev.name + ")");
				break;
			}
		}
	}
	...
	boolean processExtInputEventHelper(int type) {
		...
		if (isHelperAvailable() && cc.cp.supportsGII) {
			...
			try {
				...
				vlog.info("WARNING: Could not invoke processExtInputEvent() from VNC Helper.");
				vlog.info("  Extended input device support will be disabled.");
				helperAvailable = false;
			} catch (java.lang.Exception e) {
				vlog.info("WARNING: Could not invoke processExtInputEvent() from VNC Helper:");
	...
	JScrollPane sp;
	public Toolbar tb;
	public BottomToolbar bottom_tb;
	public int dx, dy = 0;
	MacMenuBar macMenu;
	boolean canDoLionFS;
	...
	public int buttonPressType = 0, buttonReleaseType = 1, motionType = 2;
