...
#ifndef TIZEN_GESTURE_DIRECTION_ENUM
#define TIZEN_GESTURE_DIRECTION_ENUM
enum tizen_gesture_direction {
	TIZEN_GESTURE_DIRECTION_DOWN = 1,
	TIZEN_GESTURE_DIRECTION_LEFT = 2,
	TIZEN_GESTURE_DIRECTION_UP = 4,
	TIZEN_GESTURE_DIRECTION_RIGHT = 8,
};
#endif /* TIZEN_GESTURE_DIRECTION_ENUM */
...
struct tizen_gesture_interface {
	 ...
	 * @direction: (none)
	 */
	void (*grab_swipe)(struct wl_client *client,
			   struct wl_resource *resource,
	 ...
	 * @direction: (none)
	 */
	void (*ungrab_swipe)(struct wl_client *client,
			     struct wl_resource *resource,
			     uint32_t num_of_fingers,
			     uint32_t direction);
};

#define TIZEN_GESTURE_GRAB_SWIPE_NOTIFY	0
#define TIZEN_GESTURE_SWIPE	1

#define TIZEN_GESTURE_GRAB_SWIPE_NOTIFY_SINCE_VERSION	1
#define TIZEN_GESTURE_SWIPE_SINCE_VERSION	1

static inline void
tizen_gesture_send_grab_swipe_notify(struct wl_resource *resource_, uint32_t num_of_fingers, uint32_t direction, uint32_t error)
{
	wl_resource_post_event(resource_, TIZEN_GESTURE_GRAB_SWIPE_NOTIFY, num_of_fingers, direction, error);
}

static inline void
tizen_gesture_send_swipe(struct wl_resource *resource_, uint32_t mode, uint32_t num_of_fingers, int32_t sx, int32_t sy, uint32_t direction)
{
	wl_resource_post_event(resource_, TIZEN_GESTURE_SWIPE, mode, num_of_fingers, sx, sy, direction);
}
...
/**
 * tizen_viewport - the viewport for a subsurface
 * @destroy: (none)
 * @set_transform: set the transform of a subsurface
 * @set_source: set the source rectalge of a wl_buffer
 * @set_destination: set the destination rectalge in a parent surface
 ...
 * present a subsurface on screen.
 *
 ...
 * automatically by a compositor.
 *
 * The tizen_viewport is specified in the coordinates of a subsurface's
 * parent.
 *
 ...
 * applying the destination mode to a subsurface.
 *
...
struct tizen_viewport_interface {
	 ...
	 * That is, if 90 transform is setted and its subsurface has 0
	 * transform, its subsurface won't be rotated.
	 *
	...
	void (*set_source)(struct wl_client *client,
			   ...
			   uint32_t height);
	/**
	 * set_destination - set the destination rectalge in a parent
	 *	surface
	 * @x: (none)
	 * @y: (none)
	 * @width: (none)
	 * @height: (none)
	 *
	 * The destination rectangle won't be changed when a parent is
...
/**
 * tizen_destination_mode - the destination mode for a subsurface
 * @destroy: (none)
 ...
 * will be ignored.
 *
 * The destination of a subsurface is decided by the mode, ratio, scale,
