...
struct wl_input_method_context_interface {
	 ...
	 * @serial: (none)
	 */
	void (*reset_done)(struct wl_client *client,
			   struct wl_resource *resource,
			   uint32_t serial);
};
...
#define WL_INPUT_METHOD_CONTEXT_RESET_SYNC	12
...
#define WL_INPUT_METHOD_CONTEXT_RESET_SYNC_SINCE_VERSION	1
...
static inline void
wl_input_method_context_send_filter_key_event(struct wl_resource *resource_, uint32_t serial, uint32_t time, const char *keyname, uint32_t state, uint32_t modifiers)
{
	wl_resource_post_event(resource_, WL_INPUT_METHOD_CONTEXT_FILTER_KEY_EVENT, serial, time, keyname, state, modifiers);
}

static inline void
wl_input_method_context_send_reset_sync(struct wl_resource *resource_, uint32_t serial)
{
	wl_resource_post_event(resource_, WL_INPUT_METHOD_CONTEXT_RESET_SYNC, serial);
}
