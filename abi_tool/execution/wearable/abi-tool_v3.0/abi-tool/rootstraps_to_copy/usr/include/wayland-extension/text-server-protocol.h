 ...
 * @get_hide_permission: get permission to hide input panel
 * @set_capital_mode: (none)
 *
...
struct wl_text_input_interface {
	 ...
	 */
	void (*get_hide_permission)(struct wl_client *client,
				    struct wl_resource *resource);
	/**
	 * set_capital_mode - (none)
	 * @mode: (none)
	 */
	void (*set_capital_mode)(struct wl_client *client,
				 struct wl_resource *resource,
				 uint32_t mode);
};
...
#define WL_TEXT_INPUT_HIDE_PERMISSION	20
...
#define WL_TEXT_INPUT_HIDE_PERMISSION_SINCE_VERSION	1
...
static inline void
wl_text_input_send_hide_permission(struct wl_resource *resource_, uint32_t response)
{
	wl_resource_post_event(resource_, WL_TEXT_INPUT_HIDE_PERMISSION, response);
}
